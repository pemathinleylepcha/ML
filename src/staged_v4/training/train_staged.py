...
def _compute_subnet_loss(subnet_state, sequence_batch: SubnetSequenceBatch, active_timeframe: str, active_boost: float) -> torch.Tensor:
    ...
    for timeframe, state in subnet_state.timeframe_states.items():
        ...
        if not valid.any():
            continue
        dir_loss = loss_fn(state.directional_logits[valid], tf_batch.direction_labels[valid])
        total = dir_loss
        if state.entry_logits is not None and tf_batch.entry_labels is not None:
            entry_loss = loss_fn(state.entry_logits[valid], tf_batch.entry_labels[valid])
    ...
    for fold_index, split in enumerate(splits):
        ...
        for _epoch in range(training_cfg.epochs_stage1):
            ...
            for batch_no, batch_indices in enumerate(stage1_batches, start=1):
                if batch_no == 1 or batch_no % 25 == 0 or batch_no == len(stage1_batches):
                ...
                btc_sequence, btc_state, btc_active = _run_btc_forward(btc_subnet, btc_source, batch_indices, DEFAULT_SEQ_LENS, device, btc_active, batch_counter)
                loss = _compute_subnet_loss(btc_state, btc_sequence, btc_state.active_timeframe or anchor_timeframe, subnet_cfg.active_loss_boost)
        ...
        for _epoch in range(training_cfg.epochs_stage2):
            ...
            for batch_no, batch_indices in enumerate(stage2_batches, start=1):
                if batch_no == 1 or batch_no % 25 == 0 or batch_no == len(stage2_batches):
                    log_progress(logger, status_file, "stage2_fx", batch_no, len(stage2_batches), fold=fold_index, epoch=_epoch)
                fx_optimizer.zero_grad()
                with torch.no_grad():
                    _, btc_state, btc_active = _run_btc_forward(btc_subnet, btc_source, batch_indices, DEFAULT_SEQ_LENS, device, btc_active, batch_counter)
                ...
                loss = _compute_subnet_loss(fx_state, fx_sequence, fx_state.active_timeframe or anchor_timeframe, subnet_cfg.active_loss_boost)
                if not loss.requires_grad:
                    batch_counter += 1
        ...
        if training_cfg.epochs_stage3 > 0:
            ...
            for _epoch in range(training_cfg.epochs_stage3):
                ...
                for batch_no, batch_indices in enumerate(stage3_batches, start=1):
                    if batch_no == 1 or batch_no % 25 == 0 or batch_no == len(stage3_batches):
                    ...
                    loss = _compute_subnet_loss(fx_state, fx_sequence, anchor_timeframe, subnet_cfg.active_loss_boost)
                    if not loss.requires_grad:
                        batch_counter += 1
                        del btc_state, fx_sequence, fx_state, loss
                        if jit_cleanup and batch_no % 10 == 0:
        ...
        fold_backtest_cfg = deepcopy(backtest_cfg)
        ...
        fold_result = {
            ...
            "ece": binary_ece(val_prob_flat, val_dir_labels_flat, val_valid_flat),
