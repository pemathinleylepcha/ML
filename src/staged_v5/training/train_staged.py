...
from staged_v5.config import BacktestConfig, DEFAULT_SEQ_LENS, FX_TRADABLE_NAMES, GA_PARAM_SPACE, GAConfig, STGNNBlockConfig, SubnetConfig, TrainingConfig, decode_ga_genome
...
from staged_v5.utils.calibration_helpers import apply_platt_scaler, fit_platt_scaler
from staged_v5.utils.graph_helpers import build_edge_matrices
from staged_v5.utils.runtime_logging import (
    configure_logging,
...
def _ga_optimize_backtest(
    ...
    n_bars = prob_buy.shape[0]

    def _objective(genome: list[float]) -> float:
        cfg = decode_ga_genome(genome, base_cfg)
    ...
    return decode_ga_genome(result.best_genome, base_cfg)
